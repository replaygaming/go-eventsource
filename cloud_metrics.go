...
type GoogleCloudMonitoringMetrics struct {
	prefix string
	remote cloudmonitoring.Service
}

func NewMetricAgent(prefix string){
	monitor := GoogleCloudMonitoringMetrics{prefix: prefix + "."}
...
func (monitor GoogleCloudMonitoringMetrics) ClientCount(count int) {
	// n := strconv.Itoa(count)
	// s.statsd.Gauge(1, s.prefix+"connections", n)
}

func (monitor GoogleCloudMonitoringMetrics) EventDone(event eventsource.Event, duration time.Duration, eventdurations []time.Duration) {
